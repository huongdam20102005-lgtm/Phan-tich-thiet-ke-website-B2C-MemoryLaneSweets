...
| 3 | **Process Modeling & UML (Mô hình hóa Quy trình Nghiệp vụ)** | `Evidence found` | Xây dựng hệ thống 14 sơ đồ hoạt động (Activity Diagrams), 7 sơ đồ Use Case phân rã và 8 quy trình xử lý nghiệp vụ chính mô tả rạch ròi ranh giới tương tác giữa Actor và System. *(Minh chứng: Mục 3.2 Báo cáo, `docs/03-process/`)*. |
...
| 11 | **Requirement Traceability Matrix (Ma trận Truy vết Yêu cầu)** | `Evidence newly constructed` | Xây dựng ma trận RTM hoàn chỉnh liên kết 8 tầng từ BR $\rightarrow$ UR $\rightarrow$ FR $\rightarrow$ US/UC $\rightarrow$ Rule $\rightarrow$ Feature $\rightarrow$ UI $\rightarrow$ Servlet $\rightarrow$ Code, đánh dấu rõ ràng các điểm Complete và Gap. *(Minh chứng: `docs/08-traceability/`)*. |
...
Dự án thể hiện trọn vẹn năng lực của một **Senior Business Analyst / System Analyst**:
